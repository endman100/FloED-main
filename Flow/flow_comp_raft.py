...
class RAFT_bi(nn.Module):
    ...
    def forward(self, gt_local_frames, iters=20):
        ...
        gt_flows_forward = F.interpolate(gt_flows_forward.view(-1, 2, h, w),
                                scale_factor=1 / 8,
                                mode='bilinear',
                                align_corners=True,
                                recompute_scale_factor=True)
        gt_flows_forward = gt_flows_forward.view(b, l_t - 1, 2, h // 8, w // 8)
        
        gt_flows_backward = F.interpolate(gt_flows_backward.view(-1, 2, h, w),
                                scale_factor=1 / 8,
                                mode='bilinear',
                                align_corners=True,
                                recompute_scale_factor=True)
        gt_flows_backward = gt_flows_backward.view(b, l_t - 1, 2, h // 8, w // 8)
        gt_flows_forward = gt_flows_forward.half()
        gt_flows_backward = gt_flows_backward.half()
...
class FlowLoss(nn.Module):
    ...
    def forward(self, pred_flows, gt_flows, masks, frames):
        # pred_flows: b 2 t-1 h w
        
        b, l_t, c, h, w = masks.size()
        masks = F.interpolate(masks.view(-1, c, h, w),
                                scale_factor=1 / 8,
                                mode='bilinear',
                                align_corners=True,
                                recompute_scale_factor=True)
        masks = masks.view(b, l_t, c, h // 8, w // 8)
        
        pred_flows = rearrange(pred_flows, "b c f h w -> b f c h w")
        
        h, w = pred_flows.shape[-2:]
        loss = 0
        warp_loss = 0
        masks = masks[:,:-1,...]
        ...
        smooth_loss = smoothness_loss(combined_flow.reshape(-1,2,h,w), masks.reshape(-1,1,h,w))
        smooth_loss2 = second_order_loss(combined_flow.reshape(-1,2,h,w), masks.reshape(-1,1,h,w))
